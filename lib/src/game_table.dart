...
class GameTable {
  ...
  bool checkWalkable({required int mode, required Coordinate next, required Coordinate nextIfKilling, required OnWalkableAfterKilling onKilling}) {
    if (hasMen(next)) {
      if (hasMenEnemy(next)) {
...
//      listMen.add(men);
  ...
  int countMenForPlayer(int player){
    int count = 0;
  for (int row = 0; row < countRow; row++) {
    for (int col = 0; col < countCol; col++) {
      Men? men = table[row][col].men;
      if (men != null && men.player == player) {
        count++;
      }
    }
  }
  ...
  bool checkWalkableKingInBlock(int mode, Coordinate coor,
      {
        int addRow = 0, int addCol = 0,
        required OnKingWalkable onKingWalkable,
        required OnKingWalkableAfterKilling onKingWalkableAfterKilling,
        required OnKingUnWalkable onKingUnwalkable}) {
    if (!hasMen(coor)) {
      if (mode == MODE_WALK_NORMAL) {
